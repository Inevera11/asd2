...
private:
    ...
    std::unordered_map<int, int> vertex_colors;
...
public:
    ...
    // Zwraca kolor skojarzony z wierzchołkiem x
    int getVertexColor(int x) const
    {
        assert(vertex_values.count(x));
        return vertex_colors.at(x);
    }

    // Kojarzy wartość z wierzchołkiem x
    void setVertexColor(int x, int colorIdx)
    {
        vertex_colors[x] = colorIdx;
    }
    ...
    void exportToDotFile(const std::string &filename) const
    {
        ...
        file << "strict graph G {\n";
        file << "node [colorscheme=blues8]\n";

        for (const auto &[vertex_id, vertex_value] : vertex_values)
        {
            file << "    \"" << vertex_value << "\" [color=" << vertex_colors.at(vertex_id) << "]\n";
        }
